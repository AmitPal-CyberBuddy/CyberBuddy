...
| **CORS Validator** | Method-aware two-origin probe (GET baseline + analyst-selected HEAD/OPTIONS + preflight POST simulation with Origin:null per method, credentials, `Vary: Origin`, coverage matrix, highest-risk rollup); never auto-POSTs; browser fallback is single-origin and notes its limits | Python for two-origin/null/preflight proof; hosted site probes from this origin for selected methods |
